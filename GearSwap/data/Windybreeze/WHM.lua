...
function job_setup()
	...
	No_Caress = S{'Erase', 'Esuna', 'Sacrifice'}
...
function user_setup()
	...
	state.AccessionRegen = M(false, "Accession Regen Mode") -- Set to false if you don't want default to be ON
...
function init_gear_sets()
    ...
    sets.midcast.MndEnfeebles = { --Macc/EnfeebSkill/MaccSkill
		--main="Bunzi's Rod",
		--sub="Ammurapi Shield",
		main="Contemplator +1", --70/20/228
		sub="Enki Strap", --10/0/0
		ammo="Pemphredo Tathlum", --8/0/0
		head="Theophany Cap +3", --47/0/0
		body="Theo. Bliaut +3", --50/21/0
		hands="Kaykaus Cuffs +1", --53/16/0
		legs={ name="Chironic Hose", augments={'Mag. Acc.+17 "Mag.Atk.Bns."+17','Mag. Acc.+29','Phalanx +4',}}, --60/13/0
		feet="Theo. Duckbills +3", --46/21/0
		neck="Erra Pendant", --17/0/0
		waist="Obstin. Sash", --15/5/0
		left_ear="Regal Earring", --45/0/0
		right_ear="Malignance Earring", --10/0/0
		left_ring="Stikini Ring +1", --11/8/0
		right_ring={ name="Metamor. Ring +1", augments={'Path: A',}}, --11/0/0
		back="Aurist's Cape +1", --38/0/0
    }	--Macc:491 / EnfeebSkill:104 / MaccSkill:228
...
function job_pretarget(spell, action, spellMap, eventArgs)
	if spell.type == 'WeaponSkill' and spell.target.name ~= player.name then
...
function Automated_Abilities(spell, spellMap, eventArgs)
	...
	if player.sub_job == 'SCH'and not buffactive['SJ Restriction'] then
		if state.ArtsMode.current == 'Light' and not buffactive['Light Arts'] then
			if ability_recasts[228] == 0 then -- If Light Arts (228) recast is 0
		...
		elseif state.ArtsMode.current == 'Dark' and not buffactive['Dark Arts'] then
			if ability_recasts[232] == 0 then -- If Dark Arts (232) recast is 0
	...
	if spellMap == 'StatusRemoval' and not No_Caress:contains(spell.english) then	
		if spell.name == 'Cursna' then
			if ability_recasts[26] == 0 then -- If Divine Seal (26) recast is 0
				send_command('@input /ja "Divine Seal" <me>; wait 1.5;  input /ma "'..spell.name..'" '..spell.target.name)
				eventArgs.cancel = true
				return
			-- elseif state.AccessionCursna.current == 'on' then
				-- if not buffactive['Divine Seal'] and buffactive['Light Arts'] then -- If Divine Seal recast is on cooldown, then Accession
					-- local currentStrats = get_current_strategem_count()
					-- if not buffactive['Accession'] and currentStrats > 0 then -- If Accession isn't already active; and Stratgems are available
						-- send_command('@input /ja "Accession" <me>; wait 1.5;  input /ma "'..spell.name..'" '..spell.target.name)
						-- eventArgs.cancel = true
						-- return
					-- end
				-- end
			-- elseif ability_recasts[32] == 0 then
				-- send_command('@input /ja "Divine Caress" <me>; wait 1.5; input /ma "'..spell.name..'" '..spell.target.name)
				-- eventArgs.cancel = true
				-- return
			end
		elseif ability_recasts[32] == 0 then -- If Divine Caress (32) recast is 0
			send_command('@input /ja "Divine Caress" <me>; wait 1.5; input /ma "'..spell.name..'" '..spell.target.name)
			eventArgs.cancel = true
			return
		end
	end
	-- Auto Accession before Regen
	if spellMap == 'Regen' then
		if state.AccessionRegen.current == 'on' and buffactive['Light Arts'] then
			local currentStrats = get_current_strategem_count()
			if not buffactive['Accession'] and currentStrats > 0 then -- If Accession isn't already active; and Stratgems are available
				send_command('@input /ja "Accession" <me>; wait 1.5;  input /ma "'..spell.name..'" '..spell.target.name)
				eventArgs.cancel = true
...
-- Check how many Stratgems are available for use.
